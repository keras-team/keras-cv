...
from pycoco_utils import compute_pycoco_metrics
...
class COCOPyMetric(Callback):
  ...
  def _eval_batch(self, batch, index):
      ...
      source_ids = tf.strings.join([tf.strings.as_string(tf.tile(tf.constant([index]), [batch_size])), tf.strings.as_string(tf.linspace(1, batch_size, batch_size), precision=0)], separator="/")
