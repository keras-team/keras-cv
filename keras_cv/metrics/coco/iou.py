...
def _compute_single_iou(bb1, bb2):
  ...
  area_intersection = _area(intersection_left, intersection_right,
                            intersection_top, intersection_bottom)
  area_bb1 = _area(bb1[bbox.LEFT], bb1[bbox.RIGHT], bb1[bbox.TOP],
                   bb1[bbox.BOTTOM])
  area_bb2 = _area(bb2[bbox.LEFT], bb2[bbox.RIGHT], bb2[bbox.TOP],
                   bb2[bbox.BOTTOM])
...
def compute_ious_for_image(ground_truths, predictions):
  ...
  result = tf.TensorArray(
      tf.float32,
      size=predictions_size * ground_truths_size,
      dynamic_size=False)
  # populate [m, n] ious
