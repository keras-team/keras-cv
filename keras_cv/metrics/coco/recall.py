...
class COCORecall(tf.keras.metrics.Metric):
  ...
  def __init__(self, iou_thresholds=None, categories=None, **kwargs):
    ...
    iou_thresholds = iou_thresholds or [x / 100. for x in range(50, 100, 5)]
    ...
    for image in tf.range(num_images):
      ...
      for iou_idx in tf.range(num_thresholds):
        ...
        for category_idx in tf.range(num_categories):
          ...
          result = self._single_image_recall(y_true[image], y_pred[image],
                                             ious[image, :, :], iou_thr,
                                             category, image)
  ...
  def _single_image_recall(self, y_true, y_pred, iou_table, iou_thr, category,
                           image):
    # y_true: [bboxes, 5]
    # y_pred: [bboxes, 6]
...
def _count_not_matching(x, value):
  return tf.math.count_nonzero(tf.where(tf.equal(x, value), 0., 1.), axis=-1)


def _count_matching(x, value):
  return tf.math.count_nonzero(tf.where(tf.equal(x, value), 1., 0.), axis=-1)
