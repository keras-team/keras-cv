...
class CocoRecallTest(tf.test.TestCase):
  ...
  def test_no_boxes_found(self):
    ...
    y_pred = tf.constant([[[1000, 1100, 1200, 1101, category, 1.0]]],
                         dtype=tf.float32)
  ...
  def test_every_box_found(self):
    ...
    y_true = tf.constant([[[100, 101, 200, 201, category]]], dtype=tf.float32)
    # y_pred = [images, bboxes, 6], the extra dim is confidence
    y_pred = tf.constant([[[100, 101, 200, 201, category, 1.0]]],
                         dtype=tf.float32)
  ...
  def test_iou_thrs(self):
    category = -1
    y_true = tf.constant([[[100, 101, 200, 201, category]]], dtype=tf.float32)
    y_pred = tf.constant([[[130, 131, 230, 231, category, 1.0]]],
                         dtype=tf.float32)
  ...
  def test_two_categories_one_hit_one_duplicate(self):
    ...
    y_pred = tf.constant(
        [[[100, 101, 200, 201, 1, 1.0], [100, 101, 202, 201, 1, .08]]],
        dtype=tf.float32)
  ...
  def test_two_classes_two_images(self):
    y_true = tf.constant([[[100, 101, 200, 201, 1]], [[100, 101, 200, 201, 2]]],
                         dtype=tf.float32)
    y_pred = tf.constant(
        [[[100, 101, 200, 201, 1, 1.0]], [[100, 101, 200, 201, 2, 1.0]]],
        dtype=tf.float32)
