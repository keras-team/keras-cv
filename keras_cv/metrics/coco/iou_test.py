...
class IoUTest(tf.test.TestCase):
  ...
  def test_area(self):
    self.assertEqual(iou_lib._area(100., 200., 101., 201.), 10000.0)
    self.assertEqual(iou_lib._area(0., 1., 0., 1.), 1.0)
    self.assertEqual(iou_lib._area(0., 1., 0., 0.25), 0.25)
    self.assertEqual(iou_lib._area(1., 1., 0., 1.), 0.)
    self.assertEqual(iou_lib._area(0., 1., 1., 1.), 0.)
  ...
  def test_compute_single_iou_zero_overlap(self):
    ...
    self.assertAlmostEqual(
        iou_lib._compute_single_iou(bb1, far_away_bb).numpy(), 0.)
  ...
  def test_compute_ious_for_image(self):
    ...
    expected_result = np.array(
        [[iou_bb1_bb1_off, 0., 0.], [0., 1., 0.], [0., 0., 0.]],
        dtype=np.float32)

    sample_y_true = tf.constant([bb1, top_left_bounding_box, far_away_box],
                                dtype=tf.float32)
