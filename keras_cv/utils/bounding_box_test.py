...
class BBOXTestCase(tf.test.TestCase):
    ...
    def test_corner_to_xywh(self):
        self.assertAllClose(bounding_box.corners_to_xywh(self.corner_bounding_box), self.xywh_bounding_box)
        ...
        self.assertAllClose(bounding_box.corners_to_xywh(corner_bounding_box_3d), xywh_bounding_box_3d)
        ...
        self.assertAllClose(bounding_box.corners_to_xywh(padded_corner_bounding_box), padded_xywh_bounding_box)
        ...
        self.assertAllClose(
            bounding_box.corners_to_xywh(padded_corner_bounding_box_3d), padded_xywh_bounding_box_3d
        )

    def test_xywh_to_corner(self):
        self.assertAllClose(bounding_box.xywh_to_corners(self.xywh_bounding_box), self.corner_bounding_box)

        # Make sure it also accept higher rank than 2
        corner_bounding_box_3d = tf.expand_dims(self.corner_bounding_box, 0)
        xywh_bounding_box_3d = tf.expand_dims(self.xywh_bounding_box, 0)
        self.assertAllClose(bounding_box.xywh_to_corners(xywh_bounding_box_3d), corner_bounding_box_3d)
        ...
        padded_xywh_bounding_box = tf.pad(self.xywh_bounding_box, [[0, 0], [0, 2]])
        self.assertAllClose(bounding_box.xywh_to_corners(padded_xywh_bounding_box), padded_corner_bounding_box)

        # Same for higher rank
        padded_corner_bounding_box_3d = tf.expand_dims(padded_corner_bounding_box, 0)
        padded_xywh_bounding_box_3d = tf.expand_dims(padded_xywh_bounding_box, 0)
        self.assertAllClose(
            bounding_box.xywh_to_corners(padded_xywh_bounding_box_3d), padded_corner_bounding_box_3d
        )

    def test_bounding_box_padding(self):
        bounding_boxes = [[1, 2, 3, 4], [5, 6, 7, 8]]
        target_shape = [3, 4]
        result = bounding_box.pad_bounding_box_batch_to_shape(bounding_boxes, target_shape)
        ...
        # Make sure to raise error if the rank is different between bounding_box and target
        # shape
