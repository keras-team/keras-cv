...
def pad_bounding_box_batch_to_shape(bounding_boxes, target_shape, padding_values=-1):
    ...
    Sample usage:
    bounding_box = [[1, 2, 3, 4], [5, 6, 7, 8]]   # 2 bounding_boxes with with xywh or corner format.
    ...
    Args:
        ...
        target_shape: Target shape to pad bounding box to. This should have the same
            rank as the bbounding_boxs. Note that if the target_shape contains any dimension
    ...
    paddings = [
        [0, target_shape[dim] - bounding_box_shape[dim]] for dim in range(len(target_shape))
    ]
    return tf.pad(bounding_boxes, paddings, mode="CONSTANT", constant_values=padding_values)
