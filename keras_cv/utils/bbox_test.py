...
class BBOXTestCase(tf.test.TestCase):
  ...
  def setUp(self):
    super().setUp()
    self.corner_bbox = tf.constant([[10, 10, 110, 110], [20, 20, 120, 120]], dtype=tf.float32)
    self.xywh_bbox = tf.constant([[60, 60, 100, 100], [70, 70, 100, 100]], dtype=tf.float32)
  ...
  def test_corner_to_xywh(self):
    ...
    padded_corner_bbox = tf.pad(self.corner_bbox, [[0, 0], [0, 2]]) # Right pad 2 more value
    ...
    self.assertAllClose(bbox.corners_to_xywh(padded_corner_bbox_3d), padded_xywh_bbox_3d)
  ...
  def test_xywh_to_corner(self):
    ...
    padded_corner_bbox_3d = tf.expand_dims(padded_corner_bbox, 0)
    padded_xywh_bbox_3d = tf.expand_dims(padded_xywh_bbox, 0)
    self.assertAllClose(bbox.xywh_to_corners(padded_xywh_bbox_3d), padded_corner_bbox_3d)
