...
from tensorflow_datasets.proto import waymo_dataset_generated_pb2 as dataset_pb2
...
WOD_FRAME_OUTPUT_SIGNATURE = {
    ...
    "point_smooth_xyz": tf.TensorSpec([None, 3], tf.float32),
    "point_feature": tf.TensorSpec([None, 4], tf.float32),
    "point_mask": tf.TensorSpec([None], tf.bool),
    "point_ri_row_col_sensor_id": tf.TensorSpec([None, 3], tf.float32),
...
def _get_point_top_lidar(
    ris: Sequence[tf.Tensor], frame: dataset_pb2.Frame
) -> struct.PointTensors:
    """Gets point related tensors for the top lidar.

    Args:
      ris: range image tensors. Must have 2 returns. The range image is: [range,
    ...
    return struct.PointTensors(
        point_smooth_xyz=xyz,
        point_feature=feature,
        point_ri_row_col_sensor_id=ri_row_col_sensor_id,
        label_point_nlz=nlz,
    )
...
def _downsample(point: struct.PointTensors, n: int) -> struct.PointTensors:
    ...
    num_points = point.point_smooth_xyz.shape[0]
    if num_points <= n:
        return point
...
def _get_point(
    frame: dataset_pb2.Frame, max_num_lidar_points: int, is_sdsd: bool = False
) -> struct.PointTensors:
...
def _get_label_point(
    box: tf.Tensor, box_class: tf.Tensor, point_smooth_xyz: tf.Tensor
) -> tf.Tensor:
    ...
    Args:
      ...
      point_smooth_xyz: [N, 3] points.

    Returns:
      point_box_class: [N] box class of each point.
    """
    n = point_smooth_xyz.shape[0]
    m = box.shape[0]
    if m == 0:
        return tf.zeros([n], dtype=tf.int32)

    # [N, M]
    point_in_box = box_utils.is_within_box_3d(point_smooth_xyz, box)
    # [N]
...
def _get_point_label(
    frame: dataset_pb2.Frame, point_smooth_xyz: tf.Tensor
) -> struct.LabelTensors:
    """Extracts labels.

    Args:
      frame: an open dataset frame.
      point_smooth_xyz: [N, 3] tensor representing smooth point xyz.
    ...
    point_box_class = _get_label_point(box_3d, box_class, point_smooth_xyz)
...
def point_vehicle_to_smooth(
    point_vehicle_xyz: tf.Tensor, sdc_pose: tf.Tensor
) -> tf.Tensor:
    """Transforms points from vehicle to smooth frame.
...
def box_3d_vehicle_to_smooth(box_3d: tf.Tensor, sdc_pose: tf.Tensor) -> tf.Tensor:
    """Transforms 3D boxes from vehicle to smooth frame.
    ...
    Returns:
      The boxes in smooth frame.
    """
    center = box_3d[..., 0:3]
    dim = box_3d[..., 3:6]
    heading = box_3d[..., 6]

    new_center = point_vehicle_to_smooth(center, sdc_pose)
    new_heading = (
...
def build_tensors_from_wod_frame(frame: dataset_pb2.Frame) -> dict[str, tf.Tensor]:
    ...
    point_label_tensors = _get_point_label(frame, point_tensors.point_smooth_xyz)

    # Transforms lidar frames to smooth coordinates.
    point_tensors.point_smooth_xyz = point_vehicle_to_smooth(
        point_tensors.point_smooth_xyz, pose
    )
    point_label_tensors.label_box = box_3d_vehicle_to_smooth(
        point_label_tensors.label_box, pose
    )

    # Constructs final results.
    num_points = point_tensors.point_smooth_xyz.shape[0]
    return {
        "frame_id": frame_id,
        "timestamp_offset": tf.constant(0.0, dtype=tf.float32),
        "timestamp_micros": timestamp_micros,
        "pose": pose,
        "point_smooth_xyz": point_tensors.point_smooth_xyz,
        "point_feature": point_tensors.point_feature,
        "point_mask": tf.ones([num_points], dtype=tf.bool),
        "point_ri_row_col_sensor_id": point_tensors.point_ri_row_col_sensor_id,
