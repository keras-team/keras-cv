...
class MaskRCNN(Task):
    ...
    This model is compatible with Keras 3 only. Mask R-CNN is an extension of
    Faster R-CNN, providing an additional mask head that predicts segmentation
    masks. The constructor requires `num_classes`, `bounding_box_format`,
    ...
    def __init__(
        ...
        **kwargs,
        ...
        anchor_generator = (
            anchor_generator
            or MaskRCNN.default_anchor_generator(
                fpn_min_level,
                fpn_max_level + 1,
                anchor_scales,
    ...
    def compile(
        ...
        **kwargs,
        ...
        # We define a `compute_loss` method, which performs all steps necessary
        # for computing the `rpn_box`, `rpn_classification`, `box`,
        # `classification` and `segmask` outputs. We will then arrange these
        # outputs in a dictionary and specify the relevant losses in a
        # dictionary to have Keras compute the loss for each of the outputs.
        if loss is not None:
            raise ValueError(
    ...
    def prediction_decoder(self, prediction_decoder):
        if prediction_decoder.bounding_box_format != self.bounding_box_format:
            raise ValueError(
                "Expected `prediction_decoder` and MaskRCNN to "
    ...
    def decode_predictions(self, predictions, images):
        ...
        y_pred["segmask"] = self.decode_segmentation_masks(
            ...
            bbox_format=self.bounding_box_format,
            image_shape=image_shape,
        )
        return y_pred
