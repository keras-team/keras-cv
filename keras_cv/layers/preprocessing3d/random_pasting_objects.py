...
class RandomPastingObjects(base_augmentation_layer_3d.BaseAugmentationLayer3D):
    ...
    def __init__(
        self, label_index=None, min_paste_bounding_boxes=0, max_paste_bounding_boxes=10, **kwargs
    ):
        ...
        **kwargs
        ...
        if self._label_index:
            object_mask = tf.math.equal(
                object_bounding_boxes[0, :, 7], self._label_index
            )
        ...
        num_compare_boounding_boxes = tf.math.minimum(
            num_paste_bounding_boxes * 5,
            tf.shape(object_point_clouds)[1],
        )

        object_point_clouds = object_point_clouds[:, :num_compare_boounding_boxes, :]
        object_bounding_boxes = object_bounding_boxes[
            :, :num_compare_boounding_boxes, :
        ]
        # Use the current frame to check overlap between pasted bounding boxes and existing bounding boxes
        all_bounding_boxes = tf.concat([bounding_boxes, object_bounding_boxes], axis=1)
        iou = self._iou_3d(all_bounding_boxes[0, :, :7], all_bounding_boxes[0, :, :7])
    ...
    def augment_point_clouds_bounding_boxes(
        ...
        object_point_clouds = transformation["object_point_clouds"]
        object_bounding_boxes = transformation["object_bounding_boxes"]
        ...
        points_in_paste_bounding_boxes = tf.reduce_any(
            is_within_box3d(point_clouds[:, :, :3], object_bounding_boxes[:, :, :7]),
            axis=-1,
        )
        ...
        for frame_index in range(num_frames):
            ...
            paste_point_clouds = tf.boolean_mask(
                object_point_clouds[frame_index],
                tf.math.greater(object_point_clouds[frame_index, :, :, 3], 0.0),
                axis=0,
            )
            point_clouds_list += [
            ...
            existing_bounding_boxes = tf.boolean_mask(
                ...
                tf.math.greater(bounding_boxes[frame_index, :, 7], 0.0),
            )
            paste_bounding_boxes = tf.boolean_mask(
                object_bounding_boxes[frame_index],
                tf.math.greater(object_bounding_boxes[frame_index, :, 7], 0.0),
                axis=0,
            )
            bounding_boxes_list += [
