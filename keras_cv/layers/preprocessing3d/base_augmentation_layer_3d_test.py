...
class RandomAddLayer(BaseAugmentationLayer3D):
    ...
    def get_random_transformation(self, **kwargs):
        random_x = self._random_generator.random_normal((), mean=0.0, stddev=self._translate_noise[0])
        random_y = self._random_generator.random_normal((), mean=0.0, stddev=self._translate_noise[1])
        random_z = self._random_generator.random_normal((), mean=0.0, stddev=self._translate_noise[2])
    ...
    def augment_point_clouds_bounding_boxes(self, point_clouds, bounding_boxes, transformation, **kwargs):
        ...
        return (tf.concat([point_clouds_xyz, point_clouds[..., 3:]], axis=-1), 
...
class BaseImageAugmentationLayerTest(tf.test.TestCase):
    ...
    def test_augment_leaves_extra_dict_entries_unmodified(self):
        ...
        inputs = {POINT_CLOUDS: point_clouds, BOUNDING_BOXES: bounding_boxes, "dummy": dummy}
        outputs = add_layer(inputs)
