...
class RandomBrightness(tf.keras.layers.Layer):
  ...
  Args:
    scale: Float or a list/tuple of 2 floats between -1.0 and 1.0. The scale is
      used to determine the lower bound and upper bound of the brightness adjustment.
      A float value will be choose randomly between the limits. When -1 is chosen,
      the output image will be black, and when 1.0 is chosen, the image will be
      fully white. When only one float is provided, eg, 0.2, then -0.2 will be used
      for lower bound and 0.2 will be used for upper bound.
    seed: integer, for fixed RNG behavior.

  Inputs:
    3D (HWC) or 4D (NHWC) tensor, with float or int dtype. The value should be ranged
    between [0, 255].

  Output:
    3D (HWC) or 4D (NHWC) tensor with brightness adjusted based on the `scale`. The
    output will have same dtypes as the input image.
