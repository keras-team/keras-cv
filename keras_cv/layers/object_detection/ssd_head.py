...
class SSDHead(tf.keras.layers.Layer):
    ...
    def __init__(self, num_anchors, num_classes, **kwargs):
        ...
        self.classification_head = self.conv_ssd_head(
            self.num_anchors, self.num_classes, name="cls"
        )
        self.regression_head = self.conv_ssd_head(
    ...
    def call(self, x):
        ...
        for i, features in enumerate(x):
            ...
            cls_result = self.reshape_features(cls_result, self.num_classes)
            bbox_result = self.reshape_features(bbox_result, 4)

            classification_results.append(cls_result)
            bbox_results.append(bbox_result)

        classification_results = tf.concat(classification_results, axis=1)
        bbox_results = tf.concat(bbox_results, axis=1)
