...
class EfficientNetBlock(layers.Layer):
    ...
    def conv_bn(
        ...
        Args:
            ...
            kernel_initializer: dict (or) str (or) callable, The kernel
            for Conv layer.
            bn_norm: bool, Boolean to add BatchNormalization layer after
            Conv layer.
            activation: str (or) callable, Activation to be applied on the
            output at end.
    ...
    def __call__(self, inputs):
        ...
        if 0 < self.se_ratio <= 1:
            ...
            se = layers.GlobalAveragePooling2D(name=self.name + "_se_squeeze")(
                x
            )
            if BN_AXIS == 1:
        ...
        if (
            self.id_skip
            and self.strides == 1
            and self.filters_in == self.filters_out
        ):
            if self.drop_rate > 0:
                x = layers.Dropout(
                    self.drop_rate,
                    noise_shape=(None, 1, 1, 1),
                    name=self.name + "_drop",
                )(x)
...
class EfficientNet(keras.Model):
    ...
    Args:
        ...
        to use on the "top" layer. Ignored unless `include_top=True`. Set
            `classifier_activation=None` to return the logits of the "top"
            layer.
    Returns:
      A `keras.Model` instance.
