...
def ConvBlock(filters, kernel_size, strides, padding, name=None):
    ...
    """
    def apply(x):
        x = tf.keras.layers.Conv2D(filters=filters, kernel_size=kernel_size, strides=strides, padding=padding, name=name + "_0_conv")(x)
...
def ResNeXt_Bottleneck(inputs, filters, strides, groups, bottleneck_width, name=None):
    ...
    x = ConvBlock(inputs=inputs, filters=filters, kernel_size=1, strides=1, padding="same", name = name + 'conv_block_1')
    x = GroupConv2D(
    ...
    shortcut = ConvBlock(inputs, filters=2 * filters, kernel_size=(1, 1), strides=strides, padding="same",name = name + 'conv_block_3')
    outputs = tf.keras.layers.add([x, shortcut])
    return outputs


def ResNeXt_Block(inputs, filters, strides, groups, num_blocks, bottleneck_width, name=None):
...
def ResNeXt(
    ...
    x = tf.keras.layers.Conv2D(
        filters=64, kernel_size=(7, 7), strides=2, padding="same",name="post_conv2d")(x)
    x = tf.keras.layers.BatchNormalization(name="post_bn")(x)
    x = tf.keras.activations.relu(name="post_relu")(x)
    x = tf.keras.layers.MaxPool2D(pool_size=(3, 3), strides=2, padding="same", name="max_pool")(x)
