...
def get_vgg_layers(block_name, input_shape):
    vgg16 = VGG16(
        include_rescaling=False, include_top="False", input_shape=(224, 224, 3)
    )
...
def fcn8(n_classes):
    ...
    x1 = tf.keras.layers.Conv2DTranspose(
        n_classes, kernel_size=(16, 16), strides=(8, 8), use_bias=False
    )(x1)
...
def fcn16(n_classes):
    ...
    x1 = tf.keras.layers.Add()([x1, x2])
    x1 = tf.keras.layers.Conv2DTranspose(
        n_classes, kernel_size=(32, 32), strides=(16, 16), use_bias=False
    )(x1)
    model = tf.keras.Model(inputs=vgg.inputs, outputs=x1)
    return model
...
def fcn32(num_classes):
    vgg_layer_pool_5, vgg = get_vgg_layers("block5_pool", (224, 224, 3))
    x = tf.keras.layers.Conv2D(4096, (7, 7), activation="relu", padding="same")(
        vgg_layer_pool_5
    )
    x = tf.keras.layers.Dropout(0.5)(x)
    x = tf.keras.layers.Conv2D(4096, (1, 1), activation="relu", padding="same")(x)
    x = tf.keras.layers.Dropout(0.5)(x)
    x = tf.keras.layers.Conv2D(num_classes, (1, 1), activation="relu", padding="same")(
        x
    )
    x = tf.keras.layers.Conv2DTranspose(
        num_classes, kernel_size=(64, 64), strides=(32, 32), use_bias=False
    )(x)
