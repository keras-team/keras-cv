...
class CLIP(keras.Model):
    # TODO(lukewood): evaluate constructor params
    # TODO(lukewood): parameterize specific clip instantiations
    def __init__(
        self,
        embed_dim: int,
        # vision
        ...
        **kwargs
        ...
        self.logit_scale = tf.Variable(
            np.ones([]) * np.log(1 / 0.07), dtype=tf.float32, name="logit_scale"
        )

    def initialize_parameters(self):
        self.token_embedding.assign(
            tf.random.normal(self.token_embedding.shape, stddev=0.02)
        )
        self.positional_embedding.assign(
            tf.random.normal(self.positional_embedding.shape, stddev=0.01)
        )
        ...
        if isinstance(self.visual, ModifiedResNet):
            if self.visual.attnpool is not None:
                ...
                self.visual.attnpool.q_proj.weight.assign(
                    tf.random.normal(
                        self.visual.attnpool.q_proj.weight.shape, stddev=std
                    )
                )
                self.visual.attnpool.k_proj.weight.assign(
                    tf.random.normal(
                        self.visual.attnpool.k_proj.weight.shape, stddev=std
                    )
                )
                self.visual.attnpool.v_proj.weight.assign(
                    tf.random.normal(
                        self.visual.attnpool.v_proj.weight.shape, stddev=std
                    )
                )
                self.visual.attnpool.c_proj.weight.assign(
                    tf.random.normal(
    ...
    def build_attention_mask(self):
        ...
        mask = 1 - tf.linalg.band_part(
            tf.ones((self.context_length, self.context_length)), -1, 0
        )
    ...
    def encode_text(self, text):
        ...
        idx = tf.transpose(
            tf.stack(
                (tf.range(0, x_shape[0], dtype=tf.int64), eot_token),
                axis=0,
                name="take_features_idx",
            )
        )
    ...
    def call(self, input: Tuple[tf.Tensor, tf.Tensor]):
        ...
        # normalized features
        image_features = image_features / tf.norm(
