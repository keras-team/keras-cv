...
class VisualTransformer(keras.Model):
    def __init__(
        ...
        self.conv1 = klayers.Conv2D(
            width, patch_size, strides=patch_size, use_bias=False, name="conv1"
        )
        ...
        with tf.name_scope(name):
            self.class_embedding = tf.Variable(
                scale * tf.random.normal((width,)), name="class_embedding"
            )
    ...
    def call(self, x: tf.Tensor):
        ...
        x = tf.concat(
            [
                tf.broadcast_to(
                    tf.cast(self.class_embedding, x.dtype), (x_shape[0], 1, x_shape[-1])
                ),
                x,
            ],
            axis=1,
