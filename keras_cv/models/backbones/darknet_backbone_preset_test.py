...
from keras_cv.models.backbones.darknet_v1.darknet_v1_backbone import (
    darknet50Backbone,
)
from keras_cv.models.backbones.darknet_v1.darknet_v1_backbone import (
    darknetBackbone,
...
class darknetPresetSmokeTest(tf.test.TestCase, parameterized.TestCase):
    ...
    `pytest keras_cv/models/backbones/darknet_v1/darknet_v1_backbone_presets_test.py --run_large`
    """
    ...
    @parameterized.named_parameters(
        ("preset_with_weights", "darknet50_imagenet"),
        # ("preset_no_weights", "darknet50"),
    )
    def test_backbone_output(self):
        model = darknetBackbone.from_preset("darknet50")
        model(self.input_batch)

    def test_backbone_output_with_weights(self):
        model = darknetBackbone.from_preset("darknet50_imagenet")
    ...
    def test_applications_model_output(self):
        model = darknet50Backbone()
        model(self.input_batch)

    def test_applications_model_output_with_preset(self):
        model = darknet50Backbone.from_preset("darknet50_imagenet")
        model(self.input_batch)
    ...
    def test_unknown_preset_error(self):
        ...
        with self.assertRaises(ValueError):
            darknetBackbone.from_preset("darknet50_clowntown")

    def test_load_weights_error(self):
        # Try to load weights when none available
        with self.assertRaises(ValueError):
            darknetBackbone.from_preset("darknet50", load_weights=True)
...
class darknetPresetFullTest(tf.test.TestCase, parameterized.TestCase):
    ...
    This every presets for darknet and is only run manually.
    Run with:
    `pytest keras_cv/models/backbones/darknet_v1/darknet_v1_backbone_presets_test.py --run_extra_large`
    """
