...
flags.DEFINE_string("model_name", None, "The name of the KerasCV.model that was trained")
...
flags.DEFINE_string("contributor", None, "The GitHub username of the contributor of these results")

FLAGS = flags.FLAGS
FLAGS(sys.argv)

model_name = FLAGS.model_name or input("Input the name of the KerasCV.model that was trained\n")
...
training_script_json_path = full_training_script_path[:full_training_script_path.index("keras-cv/examples/training/")+27] + '/'.join(training_script_dirs[:2] + ["training_history.json"])
...
contributor = FLAGS.contributor or input("Input your GitHub username (or the username of the contributor, if it's not you)\n")
...
new_results = {"script": '/'.join(training_script_dirs[2:]), "validation_accuracy": max_validation_accuracy, "epochs_trained": training_epochs, "tensorboard_logs": f"https://tensorboard.dev/experiment/{tensorboard_experiment_id}/", "contributor": contributor, "args": args_dict}
